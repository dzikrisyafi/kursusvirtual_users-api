...
// query for database
const (
	queryGetUser               = `SELECT username, firstname, surname, email, role_id, department_id, image, status, date_created FROM users WHERE id=?;`
	...
	queryUpdateUser            = `UPDATE users SET username=?, firstname=?, surname=?, email=?, role_id=?, department_id=?, salt=?, password=?, image=?, status=? WHERE id=?;`
	...
	queryFindByUserAndPassword = `SELECT id, username, firstname, surname, email, role_id, department_id,image, status, date_created FROM users WHERE username=? AND password=?;`
	queryGetSalt               = `SELECT salt FROM users WHERE username=?;`
)

// data access object to make transaction into database users_db.

func (user *User) Save(status int) rest_errors.RestErr {
	...
	insertResult, saveErr := stmt.Exec(user.Username, user.Firstname, user.Surname, user.Email, user.RoleID, user.DepartmentID, user.Salt, user.Password, user.Image, status, user.DateCreated)
	if saveErr != nil {
...
func (user *User) GetAllUser() ([]User, rest_errors.RestErr) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.ID, &user.Username, &user.Firstname, &user.Surname, &user.Email, &user.RoleID, &user.DepartmentID, &user.Image, &status, &user.DateCreated); err != nil {
...
func (user *User) Update(status int) rest_errors.RestErr {
	...
	// execute a prepared query statement with the given arguments
	_, err = stmt.Exec(user.Username, user.Firstname, user.Surname, user.Email, user.RoleID, user.DepartmentID, user.Salt, user.Password, user.Image, status, user.ID)
	if err != nil {
...
func (user *User) FindByUsernameAndPassword() rest_errors.RestErr {
	...
	var status int
	if getErr := result.Scan(&user.ID, &user.Username, &user.Firstname, &user.Surname, &user.Email, &user.RoleID, &user.DepartmentID, &user.Image, &status, &user.DateCreated); getErr != nil {
